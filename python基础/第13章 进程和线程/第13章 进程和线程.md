...
- run()：默认调用传入 target 的对象，子类可以重写此方法来自定义行为。
...
if __name__ == "__main__":
    ...
    # 遍历任务列表，创建并启动自定义进程
    for idx, (file_name, content) in enumerate(file_tasks):
...
- `map(func, args)`：一键批量提交任务，`map`会自动将参数列表args中的每个元素传给任务函数func，在返回结果前会阻塞主进程；
- `apply(func[, args[, kwds]])`：使用 args 参数以及 kwds 命名参数`同步`调用 func , 在返回结果前会阻塞主进程。
- `apply_async(func[, args[, kwds[, callback[, error_callback]]]])`：使用 args 参数以及 kwds 命名参数`异步`调用 func，并立即返回一个 AsyncResult 对象，不会阻塞。AsyncResult 对象可以通过 callback 获取结果和通过 error_callback 处理异常。
...
| :--------------: | :--------------------------------: | :--------------------------------------: | :-----------------------------------------------: |
| **任务提交方式** |      批量提交（传入参数列表）      |         单个提交（传入单个参数）         |             单个提交（传入单个参数）              |
...
def count_file_chars(file_path):
    ...
    :return: 元组(文件名, 字符数)，方便主进程接收结果
    """
    try:
        print(f"正在处理文件：{file_path}")

        time.sleep(0.5) # 模拟更多耗时
        ...
        # 返回带文件名的结果，方便后续整理
        return (os.path.basename(file_path), char_count)
    except Exception as e:
        # 异常处理：文件不存在/编码错误等情况
        return (os.path.basename(file_path), f"错误：{str(e)}")
...
if __name__ == "__main__":
    ...
    # 第二步：使用进程池批量计算字符数
    start_time = time.time()  # 记录开始时间，对比效率

    # 使用with语句，自动管理进程池的关闭，无需手动close()
    with multiprocessing.Pool(processes=2) as pool:
        # 批量提交任务：map方法接收「任务函数」和「参数列表」
        # 进程池会自动分配进程处理每个文件，结果按参数顺序返回
        results = pool.map(count_file_chars, test_files)
        # 因为map方法会等待所有任务完成，所以不需要join()
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=5) as pool:
        ...
        for file_path in test_files:
            # 即使只有 1 个参数，也必须写成元组形式（末尾加逗号）
            results.append(pool.apply(count_file_chars, (file_path,)))
            # apply()：同步执行任务，并获取返回值，但会阻塞主进程，所以不用join()
...
if __name__ == "__main__":
    ...
    # 创建进程池：指定最大进程数为2（修改这个数看效果）
    # 使用with语句，自动管理进程池的关闭，无需手动close()
    with multiprocessing.Pool(processes=5) as pool:
        # 步骤1：存储异步任务对象（而非直接存储结果）
        async_task_list = []
        # 遍历文件列表，逐个提交异步任务
        for file_path in test_files:
            # 即使只有 1 个参数，也必须写成元组形式（末尾加逗号）
            async_task_list.append(pool.apply_async(count_file_chars, (file_path,)))

        # 步骤2：获取所有任务的结果（按提交顺序）
        results = []
        for async_task in async_task_list:
            # get()：阻塞主等待任务完成，并获取返回值
            # 可选：get(timeout=5) 设置超时时间，避免无限等待
            results.append(async_task.get())
            # 调用get()方法等待任务完成，阻塞主进程，所以不用join()
...
def callback_func(result):
    file_name, count = result
    print(f"回调函数：{file_name} 统计完成 → {count}")
...
if __name__ == "__main__":
    ...
    # 1.创建进程池：指定最大进程数为2（修改这个数看效果）
    pool = multiprocessing.Pool(processes=5)
    # 2.遍历文件列表，逐个提交异步任务
    for file_path in test_files:
        # 即使只有 1 个参数，也必须写成元组形式（末尾加逗号）
        pool.apply_async(count_file_chars, args=(file_path,), callback=callback_func)

    # 3.关闭进程池，不再接收新的任务，否则会报错
    pool.close()
    # 4.阻塞主进程，否则主进程会提前结束，等不到回调函数执行完毕
    pool.join()
...
- map(func, *iterables, timeout=None, chunksize=1)：一键提交任务，多个任务在线程池中并行执行。
  ...
  - chunksize ：（可选）：仅对大数据量迭代器有效，指定每次分给线程的参数块大小（提升效率）
...
<img src="images/image-20260103162603534.png" alt="image-20260103162603534" style="zoom:50%;" />

...
![image](assets/1bda9fc324c050dce529eba558ad960f4b3e82f97efbf86347753cd75327ceff.jpg)
...
![image](assets/65f53329c772e73b0f9b04eb6e43526e11112d505ea842f90e97a7ad7f655ef1.jpg)
...
![image](assets/acc11487022cc44f860c11f22ec54917db2fa795e22a8a1ba6fa42f85690db20.jpg)
...
![image](assets/10c9d38de9a3c074f8bf8e1a15cdfe2f00e5649b0d24360c85403ddda53ef2fd.jpg)
...
![image](assets/ea2696c103c57cd485098f4b54a5f936c0aaf1c375fe655a2d6b1130e5842f63.jpg)
...
![image](assets/736b6fcb167d522944be7e20c809b7cabc02c10e4d5792d2e03ee840a5e136a2.jpg)
...
一个主库；两个从库 +微信study322

主库宕机时，这两个从库都可以被选为主库。

![image](assets/e89ae6a1ac998d9146f2dfa506695b3695e1397f8e90dbdc3cc07404d3db4959.jpg)

当主库宕机后,两个从库都会进行竞选，其中一个变为主库，当原主库恢复后，作为从库加入当前的复制集群即可。

![image](assets/59b41473e0a0fd7e27f2b0b1a67d57587d7c42dfa384acdc9ff2ce07c42bc727.jpg)
...
![image](assets/c2ccc11e77437c7e00d1e638bfd302c68a64bb480bdd09ecb35ac85adc7c9fd2.jpg)
...
![image](assets/a2e60a5037f80af0db1176bb8022f11201b9a4c1dec118b9905c6a7f3e5ae70a.jpg)
...
下载 mongo 镜像，如有需求可加上版本号+微信study322

docker pull mongo专业一手超清完整

Digest: sha256:b66f48968d757262e5c29979e6aa3af944d4ef166314146e1b3a788f0d191ac3 Status: Downloaded newer image for mongo:latest docker, io/library/mongo:latest [root@localhost ~]#
...
[root@localhost ~]# mkdir -p /tmp/mongo{1..3}/data [root@localhost ~]# mkdir -p /tmp/mongo/conf [root@localhost ~1#
...
![image](assets/3122f7d183065e59cf66bf8cb735fc3ca5c70d8da580154fd18742b0b81cf0a0.jpg)
...
```
mongo-replitset:PRIMARY> db.hello()
{
    "topologyVersion": {
    "processId": ObjectId("60812c3e2c2e95dddf895c65"),
...
mongo-repliset:PRIMARY> db.blog.insert({
    "title": "MongoDB 教程",
    "description": "MongoDB 是一个 Nosql 数据库",
    "by": "我的博客",
    "url": "http://www.baiyp.ren",
...
![image](assets/8d5c36d97d40ca9c2f1fb1df4364d1191363393881bb84cd421a13c8d69f41ff.jpg)
...
![image](assets/4102d29d512df619f9efce65aac055661528ff487d5cc8b7488c93a214494980.jpg)
...
0.0.0.0:30003->27017/tcp, :::30003->27017/tcp

41 minutes ago

"docker-entrypoint. s.."

0.0.0.0:30002->27017/tcp, :::30002->27017/tcp
...
0.0.0.0:30001->27017/tcp, :::30001->27017/tcp
...
![image](assets/67865ec131a942334e3da0a815a543d14b640c36b3b704eebe9039ac3c3e1248.jpg)
...
"topologyVersion" : "processId" : 0bjectId("60812c5a2b9713afca6b3a36"), "counter" : NumberLong(7) "hosts" : [ "mongo1:27017", "mongo2:27017" "mongo3:27017' "setName" : "mongo-repliset". "setVersion" : 1 "ismaster"": true, "secondary"": false, "primary": "mongo2:27017", "me" : "mongo2:27017" "electionId": ObjectId("7fffffff0000000000000002") "lastWrite" : { "opTime" "ts" : Timestamp(1619080861, 1) "t" : NumberLong(2) iastWriteDate" : IS0Date("2021-04-22T08:41:017"). "majorityOpTime" : { "ts": Timestamp(1619080861, 1), "t" : NumberLong(2) "maiorityWriteDate" : IS0Date("2021-04-22T08:41:01Z") "maxBson0bjectSize" : 16777216 "maxMessageSizeBytes" : 48000000, "maxWriteBatchSize": 100000 "localTime" : IS0Date("2021-04-22T08:41:11.548Z"),
...
"docker-entrypoint.s. "docker-entrypoint.s. "docker-entrypoint . s..

45 minutes ago 45 minutes ago 45 minutes ago

Up 45 minutes Up 45 minutes Up 2 seconds

0.0.0.0:30003->27017/tcp, :::30003->27017/tcp 0.0.0.0:30002->27017/tcp, :::30002->27017/tcp 0.0.0.0:30001->27017/tcp, :::30001->27017/tcp
...
connecting to: mongodb://127.0.0.1:30001/test?compressors=disabled&gssapiServiceName=mongodb Implicit session: session { "id": UUID("b9a7a942-551e-4881-8296-59a56bc21d5c") ) MongoDB server version: 4.4.5

The server generated these startup warnings when booting 2021-04-22T16:42:36.750+08:00: The configured WiredTiger cache size is more than 80% of available RAM. See http://dochub.mongodb.org/core/faq-men

2021-04-22T16:42:37.647+08:00: Access control is not enabled for the database. Read and write access to data and configuration is unrestricted 2021-04-22T16:42:37.648+08:00: /sys/kernel/mm/transparent_hugepage/enabled is'always'. We suggest setting it to 'never 2021-04-22T16:42:37.648+08:00: /sys/kernel/mm/transparent hugepage/defrag is 'always'. We suggest setting it to 'never

Enable MongoDB's free cloud-based monitoring service, which will then receive and display metrics about your deployment (disk utilization, CPu, operation statistics, etc).

The monitoring data will be available on a MongoDB website with a unigue URL accessible to you and anyone you share the URL with. MongoDB may use this information to make product imnrovements and to suggest MongoDB products and denloyment ontions to vou
...
docker run --net mongo-cluster \ --restart always --name mongo4 -p 30004:27017 \ -v /tmp/mongo/mongo4/data:/data/db \ -v /tmp/mongo/conf:/etc/mongo \ -v /etc/localtime:/etc/localtime \ -d mongo -f /etc/mongo/mongodb.conf

PORTS NAMES 0.0.0.0:30004->27017/tcp, :::30004->27017/tcp mongo4 0.0.0.0:30003->27017/tcp, :::30003->27017/tcp monão3 0.0.0.0:30002->27017/tcp, :::30002->27017/tcp mongo2 0.0.0.0:30001->27017/tcp, :::30001->27017/tcp mongo1

## 从主节点新增节点

## rs.add("mongo4:27017")+微信study322
...
renliset:SECONDARY> rs isMaster( "topologyVersion" : { "processId": 0bjectId("608139d70b4b32a66c866a2c"), "counter" : NumberLong(3) "hosts" "mongo1:27017" "mongo2:27017" "mongo3:27017' "mongo4:27017' "setName" : "mongo-repliset" "setVersion" : 2] "ismaster"": false "secondary"": true "primary"': "mongo2:27017" "me"":""mongo4:27017" "lastWrite":" "opTime" : : Timestamp(1619082121. 1). "t" : NumberLong(2) iastWriteDate" : ISODate("2021-04-22T09:02:01Z"), "majorityOpTime": { "ts" : Timestamp(1619082121, 1) "t" : 'NumberLong(2) "maiorityWriteDate" : IS0Date("2021-04-22T09:02:01Z" "maxBson0biectSize" : 16777216 "maxMessageSizeBytes": 48000000. "maxWriteBatchSize" : 100000 "1ocalTime": IS0Date("2021-04-22T09:02:07 040Z")
...
mongo-repliset:SECONDARY> db.setSecondary0k()
...
![image](assets/9c08ae345fd835a6db7c0a3ecb64ebbcd3d3e673ea5bd6ca788ec0fa93c00e2a.jpg)
...
![image](assets/3b18c61e137ce37b4c386ee9a097a11033a8465e0fa8a45a0437cf754c54c297.jpg)
...
![image](assets/bc0ac0712040dce5599689bc7b5e7c865fc65b643ff26048b793d8d5b7f879fd.jpg)
...
![image](assets/37b5c3dbbdb222d9669c685d69c88736435330a901a266c511cf0a587e43b0bf.jpg)
...
![image](assets/4e4e6d8643ab940df00e309f7acf8c015596dd93336af642d50e7e77fcd257e5.jpg)
...
![image](assets/9d7c72625fd74fd4686826b407e4d67556b459c735bca2f828ce4d71e8501465.jpg)
...
![image](assets/0e6e26bba273be972677121b7fd4eb986170e27893e5241ca43e5e27dd7e2427.jpg)
...
\# 对集合所在的数据库启用分片功能+微信study322 sh.enableSharding("test")
...
mongos> db.blog.createIndex({ "by": "hashed", "likes": 1})
{
    ...
    "keyId" : NumberLong("6956106243435814933")
    }
    }
}
...
mongos> sh.shardCollection("test.blog", {"by":"hashed","likes":1})
{
    ...
    "collectionUUID": UUID("alb1acbc-94b5-469a-a55c-69584740a322"),
    "ok": 1,
...
databases:
    ...
    shard2 1
    shard3 2
    { "by" : { "$minKey" : 1 }, "likes" : { "$minKey" : 1 } } --> { "by" : NumberLong("-4581349587371747453"), "likes" : 1924 } on : shard1 Timestamp(4, 2)
    { "by" : NumberLong("-4581349587371747453"), "likes" : 1924 } --> { "by" : NumberLong("-4581349587371747453"), "likes" : 2148 } on : shard1 Timestamp(4, 3)
    ...
    { "by" : NumberLong("-1693192022260240884"), "likes" : 4970 } --> { "by" : NumberLong("8705251532973802502"), "likes" : 9658 } on : shard1 Timestamp(3, 0)
    { "by" : NumberLong("8705251532973802502"), "likes" : 9658 } --> { "by" : { "$maxKey" : 1 }, "likes" : { "$maxKey" : 1 } } on : shard2 Timestamp(3, 1) mongos> 
```
...
![image](assets/ffe10441e68419c656f0b4c1695dcb88d197fe0f5c581ede950b82c07c97a7b7.jpg)
...
[root@localhost mongo-cluster]# cp /tmp/mongo-cluster/config-server/conf/mongo.key /tmp/mongo-cluster/shard4-server/conf, [root@localhost mongo-cluster]# ls /tmp/mongo-cluster/shard4-server/conf/ mongo.key [root@localhost mongo-cluster]#
...
command: --config /data/configdb/conf/mongo.conf networks:  
    mongo-cluster-network:  
    driver: bridge 
```
...
roota69e3507d8791:/# mongo -port 27017
MongoDB shell version v4.4.5
connecting to: mongodb://127.0.0.1:27017/?compressors=disabled&gssapiServiceName=mongodb
Implicit session: session { "id" : UUID("b5fe10df-741e-450b-94eb-l39b40bc8164") }
MongoDB server version: 4.4.5
...
发起删除分片节点命令，平衡器开始自动迁移数据db, runCommand(3 removeshard:"shard4" })"msg" : "draining started successfully","state" : "started"."shard" : "shard4","note" : "you need to drop or movePrimary these databases""dbsToMove" : [ ],"ok" : 1,"operationTime" : Timestamp(1619604147, 3)"$clusterTime" : {"clusterTime" : Timestamp(1619604147, 3),"signature" : {"hash": BinData(0,"J8dEBTbKeFeawvJwT1enaljnxcU="),"keyId" : NumberLong("6956100243435814933"F
...
"shard1" "host" : "shard1/shard1-seryer1:27017,shard1-server2:27017" "state' "shard2" "host" "shard2/shard2-server1:27017,shard2-server2:27017" "state' "shard3" "host": "shard3/shard3-server1:27017,shard3-server2:27017" "state' "shard4" "host" "shard4/shard4-server1:27017,shard4-server2:27017 "state'

active mongoses: "4.4.5":3

test.blog started at Wed Apr 28 2021 18:02:42 GMT+0800 (CST Failed balancer rounds in last 5 attempts: 0 Migration Results for the last 24 hours:
...
databases: id" : "config", "primary" : "config", "partitioned" : true confia,system,sessions shard key: { " id" : 1 unique: false balancing: true chunks: shard1 264 shard2 264 shard3 264 shard4 232 too many chunks to print. use verbose if you want to force print "test" "primary"": "shard2", ""partitioned" : "true," "yersion"": "{ "µuid".: IuTD("fea4d1bd-2d92-439d-b392-f2d66e6ea046") "lastMod test, blog shard key: { "by" : "hashed", "likes" : 1 ] unique: false balancing: true chunks: shard1 shard2 à shard3 { "by" : { "$minKey" : 1 }, "likes" : { "$minKey" : 1 } } -->> { "by" : NumberLong("-4581349587371747453"), "likes" : 19 24 } on : shard2 Timestamp(5, 0) {"bý" : Numberlong("-4581349587371747453"), "likes" : 1924 } -->> { "by" : Numberlong("-4581349587371747453"), "likes : 2148 } on : shard2 Timestamp(io, 0) "{"by":NumberLong("-4581349587371747453"), "likes" : 2148 } -->> { "by" : NumberLong("-2841831389122157606"), "likes : 572 } on : shard2 Timestamp(1i, 0) { "by"":'NumberLong("-2841831389122157606"), "likes" : 572 } -->> { "by" : NumberLong("-1850026574102736967"), "likes" 4145 } on : shard3 Timestamp(12. 0 { "by":Numberlong("-1850026574102736967"). "likes" : 4145 } -->> { "by" : Numberlong("-1693192022260240884"). "likes

## 再次删除节点

真正从分片集群中删除shard副本集信息

use admin db.runCommand({removeShard:"shard4"})
...
"$clusterTime": {
    ...
    "signature": {
    "hash": BinData(0, "j5wdqhQX0ET7fr4bBR9UWgolPAQ="),
    "keyId": NumberLong("6956100243435814933")
    }
} 
```

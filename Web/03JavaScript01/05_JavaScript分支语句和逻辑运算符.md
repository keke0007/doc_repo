...
三元运算符 逻辑运算符 switch语句

## 程序的执行顺序

1在程序开发中，程序有三种不同的执行方式：

- 顺序 从上向下，顺序执行代码
- 分支 根据条件判断，决定执行代码的分支
- 循环 让特定代码重复执行
条件不成立 条件？ 条件 条件成立 条件成立 处理条件 成立执行的代码 不成立执行的代码 循环执行的代码 条件不成立 顺序 分支 循环

## 代码块的理解

代码块是多行执行代码的集合，通过一个花括号放到了一起。
...
- 小明妈妈说：如果小明考试了100分，就去游乐场场(判断分数等于100分)
...
- 转换规则和Boolean函数的规则一致；
数字O、空字符串""、null、undefined 和NaN 都会被转换成false。
...
1有时我们需要根据一个条件去赋值一个变量。

- 比如比较数字大小的时候，获取较大的数字；
- 这个时候ifelse语句就会显得过于臃肿，有没有更加简介的方法呢？
条件运算符：'？
...
运算符 运算规则 范例 结果 && 与：同时为真 false && True false 或：一个为真 false or frue true 非：取反 !false true ■有了逻辑运算符，我们就可以在判断语句中编写多个条件。

## 逻辑或的本质

```javascript
Ⅱ(或)两个竖线符号表示"或"运算符(也称为短路或)：
result = a ll b;
```
...
注意：返回的值是操作数的初始形式，不会转换为Boolean类型。 ！换句话说，一个或运算一的链，将返回第一个真值，如果不存在真值，就返回该链的最后一个值。

```javascript
varshowName=·name nickname defaultName
```
...
```javascript
1与if语句不同的是，switch语句只能做值的相等判断(使用全等运算符===)，而if语句可以做值的范围判断；
```

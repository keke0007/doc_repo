...
<img src="images/image-20251219205222688.png" alt="image-20251219205222688" style="zoom: 80%;" />
...
<img src="images/image-20251219205611529.png" alt="image-20251219205611529" style="zoom:67%;" />
...
（1）`__new__(cls, *args, **kwargs)`：对象实例化时第一个调用的方法。需要调用super().__new__(cls)创建实例对象，并返回实例对象。

（2）`__init__(self, 实例属性参数)`：类的初始化方法。
...
（8）`__repr__(self)`：默认实现会返回 `<类名 object at 0x内存地址>`。被`repr(对象)`、**Python 交互式解释器直接输入对象**、调试工具**隐式调用**。如果重写，面向**开发者**（准确可还原）。缺`__str__`时会自动调用`__repr__`，缺`__repr__`时则返回默认的内存地址格式。
...
- 动态给实例对象添加实例方法：
  ...
  - （2）通过 `类名.方法名 = types.MethodType(函数名,实例对象名)` 的方式绑定到具体的实例对象上，只有该实例对象拥有这个方法
- 动态给类添加实例方法
  - （1）在类外面定义函数，且第一个参数为self
  - （2）通过 `类名.方法名 = 函数名`的方式绑定到类上，该类所有实例对象都有这个方法
...
# 给p1对象动态添加实例方法
# p1.drink = drink # 这么添加方法，p1.eat()会报错
Person.drink = types.MethodType(drink, p1)
p1.drink()  # 张三在吃饭
...
- `__slots__`用来限制该类的实例能拥有的实例属性和实例方法。

- `__slots__`对类属性、类方法和静态方法没有限制。

- `__slots__`仅对当前类生效，对其子类无效。

```python
"""
    演示 __slots__ 变量限制动态添加实例属性和实例方法
"""
import types
...
# 添加实例属性
p.age = 10
print(f"p.age = {p.age}")  # 10

# 添加实例属性
# p.weight = 100  # 错误，weight 不在 __slots__ 中
...
# 添加实例方法
# p.drink = type.MethodType(drink, p)  # 错误，drink 不在 __slots__ 中
